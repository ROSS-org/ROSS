...
num_start(num_state * state, tw_bf * bf, rn_message * msg, tw_lp * lp)
{
	...
	if(g_num_mod == 0 && g_num_nreport_days == 0)
	{
		...
		e = rn_timer_init(lp, ((today + 1) * 86400) - 1);
		m = tw_event_data(e);
		m->ttl = 2;
	...
	if(g_num_mod > 1 && (today % g_num_mod)+1 != g_num_mod)
		return;
	if(g_num_nreport_days > 0 && today != g_num_report_days[g_num_next_report_day])
			return;
	...
	if(timeofday < NINE_OCLOCK)
	{
		ts = tw_now(lp) + NINE_OCLOCK - timeofday 
				+ tw_rand_unif(lp->rng) * ONE_MINUTE;
	...
	} else if (timeofday == NINE_OCLOCK)
	{
		...
		ts = tw_now(lp) + tw_rand_unif(lp->rng) * ONE_MINUTE;
...
{
	int today = (int) tw_now(lp) / TWENTY_FOUR_HOURS;;

	// print out final days worth of statistics
	if(g_num_mod != INT_MIN)
	{
		if(g_num_mod != 0)
		{

			// don't mess with Texas
			num_report_levels(today-(today%g_num_mod)-1+g_num_mod);
			g_num_mod = INT_MIN;
		} else
			num_report_levels(today);
	}
