...
num_file_start(num_state * state, tw_bf * bf, unsigned int size, tw_lp * lp)
{
#if NUM_VERIFY
	if(lp->id == DEBUG_LP)
		printf("%lld T%d %s: FILE_START size: %.2lfB\n", 
	...
	rn_event_send(rn_event_new(lp->gid, 0.0, lp, DOWNSTREAM, size/8));
}
...
#if NUM_VERIFY
if(lp->id == DEBUG_LP)
	printf("%lld T%d %s: FILE_STOP\n", lp->id, state->type, get_tod(tw_now(lp)));
#endif

	state->stats->s_nstop++;

	if(g_num_mod < 1 && g_num_nreport_days == 0)
		return;

	// send event down to TCP
	rn_event_send(rn_event_new(lp->gid, 0.0, lp, DOWNSTREAM, 0));
}
...
num_file_complete(num_state * state, tw_bf * bf, rn_message * msg, tw_lp * lp)
{
...
#if NUM_VERIFY
	if(lp->id == DEBUG_LP)
		printf("%lld T%d %s: FILE_COMPLETE\n", lp->id, state->type,
			 get_tod(tw_now(lp)));
#endif
	...
	if(lp->id == DEBUG_LP)
	{
		printf("%lld T%d %s: actual: %d, desired: %u, size: %.2lfB, elapsed: %.4lf\n",
	...
	{
		// Increase file size if getting more than desired
	...
	} else if (actual >= np->bitrate / 10.0)
	{
		...
		// Ask for another file of same size right away
		num_file_start(state, bf, state->file_size, lp);
	} else
	{
		// rate is less than acceptable, try again in 2 minutes
		// to keep load on network
