...
init_default_profiles()
{
	...
	g_num_nprofiles = 5;
	g_num_profiles = tw_vector_create(sizeof(num_profile), 5);
...
init_profiles_from_file()
{
	...
	g_num_profiles = tw_vector_create(sizeof(num_profile), g_num_nprofiles);
...
num_init_scenario()
{
	...
	g_num_profile_cnts = tw_vector_create(sizeof(unsigned int *), g_num_nprofiles);
	g_num_net_pop = tw_vector_create(sizeof(unsigned int *), g_num_nprofiles);

	if(g_num_mod >= 0)
	{
		// log number of network users who achieve each level by type
		g_num_level = tw_vector_create(sizeof(unsigned int *), g_num_nprofiles);

		for(i = 0; i < g_num_nprofiles; i++)
			g_num_level[i] = tw_vector_create( sizeof(unsigned int), NUM_LEVELS);
