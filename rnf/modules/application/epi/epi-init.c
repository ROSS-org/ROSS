...
init_default_ic_stages()
{
	...
	g_epi_stages = tw_calloc(TW_LOC, "", sizeof(epi_ic_stage), 1);
	...
	s = &g_epi_stages[0];
	s->stage_index = 0;
	s->stage_name = tw_calloc(TW_LOC, "", sizeof(char), 12);
...
init_ic_stages_from_file()
{
	...
	g_epi_stages = tw_calloc(TW_LOC, "", sizeof(epi_ic_stage), g_epi_nstages);
	...
	while(EOF != fscanf(f, "%u", &s->stage_index))
	{
		...
		s->stage_name = tw_calloc(TW_LOC, "", sizeof(char) * (strlen(line)+1), 1);
