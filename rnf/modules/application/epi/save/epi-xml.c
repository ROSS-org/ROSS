...
epi_xml(epi_state * state, const xmlNodePtr node, tw_lp * lp)
{
	...
	if (!g_epi_ct)
	{
		g_epi_nct = rn_getarea(m)->nsubnets;
	...
	for(agent = node->children; agent; agent = agent->next)
	{
		...
		{
			if ( tw_rand_unif(lp->id) < g_epi_psick )
				sick = "1";
		}
		...
		if(0 != strcmp(sick, "0"))
		{
			...
			x = ((double) tw_rand_integer(lp->id, 0, INT_MAX) / (double) INT_MAX);
		...
		if (seconds_used > 86400)
			tw_error(TW_LOC, "Agent %d has duration more than 24 hours\n",
					a->id);
		...
		{
			if (tw_rand_unif(lp->id) < g_epi_worried_well_rate)
				a->behavior_flags = 1;
		} else if (g_epi_work_while_sick_p > EPSILON)
			if (tw_rand_unif(lp->id) < g_epi_work_while_sick_p)
				a->behavior_flags = 2;
