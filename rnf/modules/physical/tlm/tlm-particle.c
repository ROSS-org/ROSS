...
#define VERIFY_PARTICLE 1
	 ...
	 */
tw_event	*
tlm_particle_send(tw_lpid gid, tw_stime offset, tw_lp * src_lp)
{
	tw_event	*e;
	//tw_memory	*b;
...
#if DWB
	b = tw_memory_alloc(src_lp, g_tlm_fd);
...
void
tlm_particle_handler(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	//tw_memory	*b;
	//tw_memory	*b_in;
	...
	if(state->prev_time != tw_now(lp))
	{
		...
		{
			bf->c1 = 1;
			tw_memory_free(lp, state->initial, g_tlm_fd);
			state->initial = NULL;
		}
	...
	{
		bf->c0 = 1;
		tlm_proximity_send(state, b_in, state->initial, lp);

		tw_memory_free(lp, b_in, g_tlm_fd);
		tw_memory_free(lp, state->initial, g_tlm_fd);
		state->initial = NULL;
	} else
	...
	position = tlm_getlocation(lp);
	fprintf(g_tlm_parts_plt_f, "%lf %ld %lf %lf %ld\n", tw_now(lp), 
		p->user_lp->id, position[0], position[1], lp->id);
#endif
}

void
tlm_rc_particle_handler(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	//tw_memory	*b;
	//tw_memory	*b_in;
	...
	if(bf->c0 == 1)
	{
		state->initial = tw_memory_free_rc(lp, g_tlm_fd);
		tw_event_memory_get_rc(lp, tw_memory_free_rc(lp, g_tlm_fd), g_tlm_fd);
	} else
	{
		tw_event_memory_get_rc(lp, state->initial, g_tlm_fd);
		state->initial = NULL;
	...
	//tw_event_memory_get_rc(lp, b_in, g_tlm_fd);
	...
	if(bf->c0 == 1)
	{
		...
		while((NULL != (b = tw_memory_free_rc(lp, g_tlm_fd))))
