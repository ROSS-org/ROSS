...
#define VERIFY_RM_CELL 0
	 ...
	 */
void
tlm_rc_cell_handler(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	switch(m->type)
	{
		case RM_SCATTER:
			tlm_rc_scatter(state, bf, m, lp);
			state->stats->s_ncell_scatter--;
			break;

		case RM_GATHER:
			tlm_rc_gather(state, bf, m, lp);
...
void
tlm_rc_scatter(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	int	 i;
...
void
tlm_rc_gather(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	int	 i;
