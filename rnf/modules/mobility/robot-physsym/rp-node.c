...
#endif
void
change_direction(rp_state * state, rp_message * m, tw_bf * bf, tw_lp * lp)
{
	state->theta = tw_rand_integer(lp->rng, 0, 7);
	state->change_dir = tw_rand_integer(lp->rng, g_rp_mu, g_rp_distr_sd) +
				tw_now(lp);
...
inline void
new_location(rp_state * state, tw_bf * bf, tw_lp * lp)
{
...
inline void
rc_new_location(rp_state * state, tw_bf * bf, tw_lp * lp)
{
	state->position[0] -= state->velocity[0];
	state->position[1] -= state->velocity[1];
}
...
/*
 * rp_move - compute next location for this node
 */
void
rp_move(rp_state * state, rp_message * m, tw_bf * bf, tw_lp * lp)
{
	tw_event	*e;
	tw_memory	*b;
...
void
rp_rc_move(rp_state * state, rp_message * m, tw_bf * bf, tw_lp * lp)
{
	int	 n = * (int *) bf;
	int	 i;
