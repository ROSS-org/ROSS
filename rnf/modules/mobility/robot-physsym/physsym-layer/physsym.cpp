...
int physSimControl(struct _physControlInput* inp, struct _physControlOutput* outp,struct _robot* r)
{
	...
	{
			btTransform trans;
			bodyBox->getMotionState()->getWorldTransform(trans);
			outp->endX = trans.getOrigin().getX();
			outp->endY = trans.getOrigin().getY();
			outp->endZ = trans.getOrigin().getZ();
	}
