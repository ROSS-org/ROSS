...
read_lat_file()
{
	...
	while(1)
	{
		if(EOF == (rv = fscanf(g_lat_file, "%s %s %d\n", 
				src, dst, &latency)))
			break;

		printf("src: %d dst: %d latency: %d \n",
...
/*
		for(i = 0; i < g_node_count; i++)
		{
printf("Checking node: %d, num links %d (%d)\n", 
			g_node_list[i].id, g_node_list[i].num_links,
			g_node_count);

			if(g_node_list[i].id == src_id)
			{
				n = &g_node_list[i];
				break;
			}
...
*/
		n = &g_node_list[src_id];
		printf("node %d has %d links \n", src_id, n->num_links);
		for(i = 0; i < n->num_links; i++)
		{
			if(n->link_list[i].node_id == dst_id)
			{
				printf("found link \n");
			}
		}
...
read_rocketfuel_file(char **argv)
{
	...
	if(NULL == (g_lat_file = fopen(argv[3], "r")))
	{
		printf ("Failed to open latency file %s for reason %s \n",
...
*/
	...
	for(i = 0; i < g_total_routers; i++)
		max_links += g_node_list[i].num_links;
	...
	for (i = 0; i < g_node_count; i++)
	{
		...
		for (j = 0; j < n->num_links; j++)
		{
		int a = g_node_list[n->link_list[j].node_id].id;
...
main(int argc, char **argv)
{
	if (3 > argc)
	{
		printf("usage: convert output.xml input.cch\n");
		exit(1);
	}
