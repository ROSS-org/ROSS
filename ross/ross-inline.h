...
static inline tw_event* tw_event_grab(tw_pe *pe)
{
...
static inline tw_event* tw_event_new(tw_lpid dest_gid, tw_stime offset_ts, tw_lp * sender) {
  tw_pe		*send_pe;
  tw_event	*e;
  tw_stime	 recv_ts;
  ...
  while(m)
      ...
      if(0 == --m->nrefs)
	{
	  if(e->state.owner >= TW_net_outq && e->state.owner <= TW_pe_sevent_q)
	    tw_memory_unshift(e->src_lp_ptr, m, m->fd);
	  else
	    tw_memory_unshift(e->dest_lp_ptr, m, m->fd);
	}
...
tw_event_memory_setfifo(tw_event * e, tw_memory * m, tw_fd fd)
{
  ...
  if(e == e->src_lp_ptr->pe->abort_event)
    {
      tw_memory_alloc_rc(e->src_lp_ptr, m, fd);
      return;
    }
...
tw_event_memory_forward(tw_event * e)
{
  ...
  e->memory = e->src_lp_ptr->pe->cur_event->memory;
