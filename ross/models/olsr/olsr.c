...
void olsr_region_event_handler(olsr_region_state * s, tw_bf * bf, olsr_message * m, tw_lp * lp) 
{
  switch( m->type ) 
    {
	case OLSR_HELLO_PACKET_SEND:

	case OLSR_TC_PACKET_SEND:

	case OLSR_DATA_PACKET_SEND:
	
    case OLSR_HELLO_PACKET_RECV:
       
    case OLSR_TC_PACKET_RECV:
      
    case OLSR_DATA_PACKET_RECV:
