...
@delays = qw(.001 .002 .003 .005 .005 .005 .005 .005 .005 .005 .005);
@bandwidths = qw(9920000000 2480000000 620000000 155000000 45000000 1500000 1500000 1000000 1000000);

#@delays = qw(.01 .02 .03 .04 .05 .06 .07 .08 .09 .10 .11);
#@bandwidths = qw(9920000000 2480000000 620000000 155000000 45000000 2500000 1500000 1000000 1000000);
...
sub print
{
	...
	{
		$bw = $bandwidths[$$x->{level}];
		$delay = $delays[$$x->{level}];
	...
	# Add NUM layer to TCP servers
	if($n->{name} =~ /^AG_SVR/)
	{
		$ospf .= "\t\t\t\t\t\t<layer name='num' type='$n->{ag_type}'/>\n";
	}
