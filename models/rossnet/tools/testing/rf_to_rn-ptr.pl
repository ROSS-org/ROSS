...
my (@g_og, %g_ct, %g_isp_leaf, %ext_nodes, %aliases, %ases, %areas, %g_loc) = ();
my (%co_in, %g_co, @g_ha, @g_nodes, @g_ag_svr, @subnets, %g_isp_bb, %g_epi, %g_epi_work) = ();
my ($as_name, $nnodes, $n_tot_nodes, $namb, $ndisc, $n_ext, $g_next_bb) = 0;
my ($g_cnt, $g_n_extlinks, $g_nlinks, $g_nagents) = 0;
my $g_root = undef;
my $g_epi_loc = 0;
...
sub print_db
{
	...
	my $fh = new FileHandle "access/ct.tab", ">";
	open(F, "access/CT.dat") or die "Unable to open access/CT.dat: $!";

	my $i = 0;
	while(<F>)
	{
		next if !defined $g_ct{$i++};

		$fh->printf("%d $_", ${$g_ct{$i}}->{nid});
	}
	...
	$fh = new FileHandle "access/co.tab", ">";
	open(F, "access/co.csv") or die "Unable to open access/co.csv: $!";

	my $i = 0;
	while(<F>)
	{
		next if !defined $g_co{$i++};
		next if 2 != ${$g_co{$i++}}->{in_network};

		$fh->printf("%d,$_", ${$g_co{$i}}->{nid});
	}
	...
	$fh = new FileHandle "access/og.tab", ">";
	open(F, "access/Offices.dat") or die "Unable to open access/Offices.dat: $!";

	my $i = 0;
	while(<F>)
	{
		next if !defined $g_og[$i++];

		$fh->printf("%d $_", ${$g_og[$i]}->{nid});
	}
...
sub print_network
{
	...
	my $fh = new FileHandle "testing.xml", ">";
	...
	# print CO, CT, and OGR tables into files for DB
	print_db();

	# Add EPI locations to the network

	# enumerate work locs from cnt + nagents + 1 to nwork
	$g_epi_loc = $cnt + $#g_ha + 1;
	for $n (values %g_epi_work)
	{
		$$n->{nid} = $g_epi_loc++;
	}
	...
	for (keys %co_in)
	{
		...
		for $ct (values %{$$co->{links2}})
		{
			next if $$ct->{name} !~ /^CT/;

			$fh->printf("\t\t<subnet id='%d'>\n", $snid++);
			for $n (values %{$$ct->{links2}})
			{
				next if $$n == $$co;

		# home epi loc (defines agt move vector)
		$fh->printf("\t\t\t\t<node id='$cnt' links='0' type='c_host'>\n");
		$fh->printf("\t\t\t\t\t<stream port='23'>\n");
		$fh->printf("\t\t\t\t\t\t<layer name='epi'>\n");

		# multiple agents per HAR
		while(@{$$n->{agent}})
		{
			my $d = shift @{$$n->{agent}};
			my $g = shift @{$$n->{agent}};
			...
			$fh->printf("\t\t\t\t\t\t\t<agent sick='0' num='$$d->{nid}' type='$t'>\n");
			$fh->printf("\t\t\t\t\t\t\t\t<move loc='$cnt' dur='9'/>\n");

			#if($g != -1 && $t > 1)
			if($g != -1)
			{
				$fh->printf("\t\t\t\t\t\t\t\t<move loc='$$g->{nid}' dur='8'/>\n");
			}
...
if(0)
{
	...
	# now tack on EPI work locations
	$fh->printf("\t<area id='%d'>\n", $arid++);
	$fh->printf("\t\t<subnet id='%d'>\n", $snid++);
	...
	{
		# work/school epi loc 
		$fh->printf("\t\t\t\t<node id='$$n->{nid}' links='0' type='c_host'>\n");
		$fh->printf("\t\t\t\t\t<stream port='23'>\n");
		$fh->printf("\t\t\t\t\t\t<layer name='epi'/>\n");
		$fh->printf("\t\t\t\t\t</stream>\n");
		$fh->printf("\t\t\t\t</node>\n");
	...
	print " done!\n\tGot $cnt nodes, $g_epi_home_loc EPI home locs, $g_epi_work_loc work locs, $bgp_routes eBGP links\n";
...
sub create_access
{
	...
	# read in desired COs, so we can parse out the partial topology desired
	open(F, 'co.in') or die "Unable to open co.in: $!";
	printf("%-50s %-15s\n", "\nProcessing Access:", "co.in");
	...
	while(<F>)
	{
		...
		die "No input Central Offices specified!\n" if !@_;
	...
	while(<F>)
	{
		...
		next if !defined $g_ha[$h];
		die "Bad HA: $h for agent $g_nagents\n" if !defined $g_ha[$h];

		printf("\t%-50s %-15d \n", "Configuring Agent", $g_nagents) if $g_nagents % 50000 == 0;

		my $n = new Node($n_tot_nodes++);
		$n->{type} = 'c_host';
		$n->name("AG_" . $g_nagents);
		...
		# setup for EPI locations
		push @{$$d->{agent}}, \$svr;

		if($g != -1)
		{
			if(!defined $g_epi_work{$g})
			{
				$g_epi_work{$g} = \(new Node($n_tot_nodes));
			}

			push @{$$d->{agent}}, $g_epi_work{$g};
		} else
		{
			push @{$$d->{agent}}, -1;
		}
