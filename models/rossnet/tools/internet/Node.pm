...
sub print
{
	...
	#return if $n->name eq "";# or $n->name == undef;

	#$nlinks = $n->{nlinks} + $n->{n_ext};
	$nlinks = $n->{nlinks2} + $n->{n_ext2};

#print "nlinks: $n->{nlinks2}, n_ext: $n->{n_ext2}, nlinks: $nlinks \n" if $n->{nid} == 15;
	...
	foreach $x (values %{$n->{links2}})
	{
		...
		$ospf .= "cost='10' h_int='2' h_bw='.0001' rdead_int='2' />\n";
	...
	if($n->{n_ext2} > 0)
	{
		...
		foreach $x (values %{$n->{ext2}})
		{
			...
			if(isa($x, 'Node'))
			{
				...
				$fh->printf("\t\t\t\t\t<link src='%d' addr='%d' bandwidth='$bw' delay='$delay' status='%s' />\n", 
						$n->{nid}, $x->{nid}, 
						($n->connected eq "true" ? "up" : "down"));
