...
stop_transfer(tcp_state * state, tw_bf * bf, rn_message * m, tw_lp * lp)
{
	...
	printf("%lld: STOP XFER: %d of %d bytes at %lf\n", 
		lp->id, state->unack, state->len, tw_now(lp));
#endif
...
{
#ifdef TCP_DEGUG
	printf("%ld: START XFER: %d of %d bytes at %lf\n", 
		lp->id, state->unack, m->size, tw_now(lp));
#endif
	if (state->timer)
...
#ifdef TCP_DEBUG
	printf("%lld: START XFER: %d bytes at %lf\n", lp->id, state->len, tw_now(lp));
#endif
	...
	tcp_event_send(state, lp->id, 0.0, state->connection, TCP_MTU, 0, 0, lp);
...
tcp_init(tcp_state * state, tw_lp * lp)
{
	...
	state->ssthresh = TCP_SND_WND;
	state->rto = 3.0;
	//state->host = rn_getmachine(lp->id);
	if(rn_getmachine(lp->id)->conn > lp->id)
		state->connection = rn_getmachine(lp->id)->conn;
	else
		state->connection = -1;
...
tcp_event_handler(tcp_state * state, tw_bf * bf, rn_message * m, tw_lp * lp)
{
	...
	*(int *) bf = 0;
	...
	{
#ifdef TCP_DEBUG
		printf("%lld: RTO for %lld at %lf (%ld)\n",
			lp->id, in->src, tw_now(lp), (long int) lp->pe->cur_event);
#endif
	...
	{
#ifdef TCP_DEBUG
		printf("%lld: ACK from %lld at %lf \n", lp->id, in->src, tw_now(lp));
#endif
		tcp_process_ack(state, bf, in, lp);
	} else
	{
#ifdef TCP_DEBUG
		printf("%lld: DATA from %lld at %lf \n", lp->id, in->src, tw_now(lp));
#endif
...
tcp_rc_event_handler(tcp_state * state, tw_bf * bf, rn_message * m, tw_lp * lp)
{
	...
	{
#ifdef TCP_DEBUG
		printf("%lld: RC RTO for %lld at %lf \n",
			lp->id, in->src, tw_now(lp));
#endif
	...
	{
#ifdef TCP_DEBUG
		printf("%lld: RC ACK from %lld at %lf \n",
			lp->id, in->src, tw_now(lp));
#endif
	...
	{
#ifdef TCP_DEBUG
		printf("%lld: RC DATA from %lld at %lf \n",
			lp->id, in->src, tw_now(lp));
#endif
	...
	if(b)
		tw_event_memory_set(lp->pe->cur_event, b, g_tcp_fd);
}
