...
tcp_update_rtt(tcp_state * state, tw_bf * bf, tcp_message * in, tw_lp * lp)
{
	...
	if(state->timer && state->timer->recv_ts > tw_now(lp))
		ts = state->timer->recv_ts;
	else
		ts = -1.0;
...
#ifdef TCP_DEBUG
//if(lp->id == 9377)
	printf("\t%lld: update RTT tmr to %lf (was %lf) (unack %d)\n", lp->id, tw_now(lp) + state->rto, ts, state->unack);
#endif
}
