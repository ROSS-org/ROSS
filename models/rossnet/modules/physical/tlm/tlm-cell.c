...
#define VERIFY_RM_CELL 0
	 ...
	 */
void
tlm_cell_handler(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	switch(m->type)
	{
	 ...
	 */
void
tlm_cell_scatter(tlm_state * state, tlm_message * m, tw_bf * bf, tw_lp * lp)
{
	tw_event	*e = NULL;
...
#if PLOT
	double		*pos = tlm_getlocation(lp);
	rpe = tw_pe_data(lp->pe);
	fprintf(rpe->wave_log, "%lf %ld %ld %lf %lf %lf %lf\n", 
	...
	for(i = 0; i < g_tlm_spatial_dir; i++)
	{
		...
		d = (g_tlm_spatial_coeff * state->displacement) - state->displacements[i];

		// Ground reflection: attenuate using ground coeff
		if((i % 2 == 0 && state->nbrs[i] == state->nbrs[i+1]) ||
			(i % 2 == 1 && state->nbrs[i] == state->nbrs[i-1]))
			d *= g_tlm_spatial_ground_coeff;

		// do not scatter below a given threshold
		if(d <= g_tlm_wave_threshold)
			continue;

		//e = tw_event_new(state->nbrs[i], 0.1, lp);
		e = tw_event_new(state->nbrs[i], g_tlm_spatial_offset_ts[i] - g_tlm_scatter_ts, lp);
...
tlm_cell_gather(tlm_state * state, tw_bf * bf, tlm_message * m, tw_lp * lp)
{
	...
	{
		e = tw_event_new(lp->gid, g_tlm_scatter_ts, lp);
	...
	m->disp[i] = state->displacements[i];
	state->displacements[i] = m->displacement;// * g_tlm_wave_loss_coeff;
