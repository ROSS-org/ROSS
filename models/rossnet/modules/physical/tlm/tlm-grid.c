 ...
 * dimension is given by the user, and stored in the array: g_tlm_spatial_d.  
 * 
 * The grid is constructed by reading in a file of Z-values given in an XY-plane.
 * The analogy is that you can think of the g_tlm_spatial_terrain_fn file as 
 ...
 * be neighbors, because they may contain Z-values > g_tlm_spatial_d[2].  Each
...
/*
 * tlm_grid_terrain(): Add terrain Z-values to the grid
 */
tw_lpid
tlm_grid_init()
{
	...
	/*
	 * tlm_grid_init(): Initialize the grid data structure
	 */
	...
	for(i = 0, nlp = 1; i < g_tlm_spatial_dim; i++)
	{
		...
		for(j = i - 1; j >= 0; j--)
		{
			g_tlm_spatial_grid_i[i] ? 
				g_tlm_spatial_grid_i[i] *= g_tlm_spatial_grid[j] :
				(g_tlm_spatial_grid_i[i] = g_tlm_spatial_grid[j]);
		}

		g_tlm_spatial_offset_ts[i*2] = g_tlm_spatial_d[i] / g_tlm_wave_velocity;
		g_tlm_spatial_offset_ts[(i*2)+1] = g_tlm_spatial_d[i] / g_tlm_wave_velocity;

//tw_printf(TW_LOC, "i %dD = %12.12lf offset ts\n", i, g_tlm_spatial_offset_ts[i*2]);

		if(0.0 > g_tlm_spatial_offset_ts[i*2] ||
		   0.0 > g_tlm_spatial_offset_ts[(i*2)+1])
			tw_error(TW_LOC, "Wave offset ts == 0.0!");

		g_tlm_scatter_ts = min(g_tlm_scatter_ts, g_tlm_spatial_offset_ts[i*2]);
	}

	g_tlm_scatter_ts /= 10.0;

	if(npe > g_tlm_spatial_grid[0])
		tw_error(TW_LOC, "Too many CPUs for this terrain!");

	f = fopen(g_tlm_spatial_terrain_fn, "r");

	if(!f)
		tw_error(TW_LOC, "Unable to open terrain file!");

	// if I am first PE, then start_row == 0
	if(!g_tw_mynode)
	{
		start_row = 0;
	} 

	// single CPU case, take all rows
	if(g_tw_mynode == tw_nnodes() - 1)
	{
		end_row = g_tlm_spatial_grid[0];
	}
	...
	// make this scalable
	g_tlm_z_values = tw_calloc(TW_LOC, "z values ptr", sizeof(g_tlm_z_values), 
					nrows_per_pe + 2);
	...
	for(x = 0; x < nrows_per_pe; x++)
		g_tlm_z_values[x] = tw_calloc(TW_LOC, "z values", sizeof(int), g_tlm_spatial_grid[1]);

	//tw_printf(TW_LOC, "%d: npe %d, nrows %d, srow %d, erow %d,  grid[1] %d\n", g_tw_mynode, npe, nrows_per_pe, start_row, end_row, g_tlm_spatial_grid[1]);

	//printf("%d: sr %d, er %d, grid0 %d, grid1 %d \n", g_tw_mynode, start_row, end_row, g_tlm_spatial_grid[0], g_tlm_spatial_grid[1]);

	// seek to correct location in file
	for(x = 0; x < start_row; x++)
	{
		for(y = 0; y < g_tlm_spatial_grid[1]; y++)
		{
			cnt++;
			if(EOF == fscanf(f, "%*d"))
	...
	for(x = start_row; x < end_row; x++)
	{
		...
		for(y = g_tlm_spatial_grid[1]-1; y >= 0; y--)
		{
			cnt++;
			if(EOF == fscanf(f, "%d", &g_tlm_z_values[xi][y]))

				tw_error(TW_LOC, "%d: Malformed terrain data! %d (%d, %d, %d)", 
					g_tw_mynode, cnt, x, xi, y);

			z = lrint((double) (g_tlm_z_values[xi][y] / g_tlm_spatial_d[2]));
			g_tlm_z_values[xi][y] = z * g_tlm_spatial_d[2];
		}
