...
my (@ct, @co, @og) = ();
my (@co_max, @og_max) = ();
my ($nlinks, $type, $id, $bw, $capacity, $max, $cnt) = undef;

open(F, "../tools/Chicago2/Chicago2.xml") or die "Unable to open: Chicago2.xml: $!";
open(CT, ">ct.dat") or die "Unable to open ct.dat: $!";
open(CO, ">co.dat") or die "Unable to open co.dat: $!";
open(OG, ">og.dat") or die "Unable to open og.dat: $!";
...
while(<F>)
{
	...
	if($type == 3)
	{
		...
		$ct[$id] = (($nlinks - 1) * 768000) +
				(155000000 * int($nlinks / 2500));
...
while(<F>)
{
	...
	#sprintf OUT, "$type $day $id %1.14f\n", $load;
	printf OUT "$type $day $id %f\n", $load;
	#print "$type $id $capacity $max $load\n" if($load > 1.0);
}
